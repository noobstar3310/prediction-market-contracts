...
///         hardcodes two outcomes (YES/NO), this one supports any `i_outcomeSlotCount >= 2`, which
...
contract MultiOutcomeMarket is ReentrancyGuard {
    ...
    uint256 public immutable i_outcomeSlotCount;

    // ----------------------------------------------------------------------------------
    // Mutable state
    // ----------------------------------------------------------------------------------

    /// @notice Pool reserves of each outcome token held by the AMM. Length == i_outcomeSlotCount.
    ...
    /// @notice Total LP shares minted; `s_sharesOf` is each provider's slice.
    ...
    uint256 public s_collectedFees;

    /// @notice Cumulative fees credited per LP share, scaled by WAD (MasterChef-style accumulator).
    uint256 public s_accFeePerShare;

    /// @notice Per-LP baseline so freshly-minted shares carry no claim on previously-accrued fees.
    mapping(address => uint256) public s_feeDebt;

    /// @notice Internal ledger of outcome tokens: s_balanceOf[outcomeIndex][account].
    mapping(uint256 => mapping(address => uint256)) public s_balanceOf;

    /// @notice Payout vector set at resolution: numerator per outcome. Length == i_outcomeSlotCount.
    /// @dev Empty until resolved. An outcome token of index i redeems for
    ///      `balance * s_payoutNumerators[i] / s_payoutDenominator` collateral.
    uint256[] public s_payoutNumerators;

    /// @notice Sum of s_payoutNumerators. Zero == unresolved; non-zero == resolved (the resolution flag).
    uint256 public s_payoutDenominator;
    ...
    constructor(IERC20 collateral_, address resolver_, uint256 closeTime_, uint16 feeBps_, uint256 outcomeSlotCount_) {
    ...
    modifier whenOpen() {
        if (block.timestamp >= i_closeTime) revert MultiOutcomeMarket__TradingClosed();
        _;
    }
    ...
    function split(uint256 amount) external nonReentrant {
        ...
        uint256 n = i_outcomeSlotCount;
        for (uint256 i = 0; i < n; i++) {
            s_balanceOf[i][msg.sender] += amount;
        }
    ...
    function merge(uint256 amount) external nonReentrant {
        ...
        for (uint256 i = 0; i < n; i++) {
            if (s_balanceOf[i][msg.sender] < amount) revert MultiOutcomeMarket__InsufficientBalance();
        }
    ...
    function addLiquidity(uint256 amount) external nonReentrant whenOpen {
        ...
        if (s_totalShares == 0) {
            ...
            sharesMinted = amount * s_totalShares / poolWeight; // round DOWN (pool's favor)

            for (uint256 i = 0; i < n; i++) {
                uint256 keep = amount * s_reserves[i] / poolWeight; // keep ratios → preserve prices
                s_reserves[i] += keep;
                // Surplus this deposit minted but the pool didn't keep is returned as an outcome token.
                s_balanceOf[i][msg.sender] += amount - keep;
            }
        }

        s_totalShares += sharesMinted;
        s_sharesOf[msg.sender] += sharesMinted;
        s_feeDebt[msg.sender] += sharesMinted * s_accFeePerShare / WAD;
    ...
    function removeLiquidity(uint256 sharesToBurn) external nonReentrant {
        ...
        if (sharesToBurn > s_sharesOf[msg.sender]) revert MultiOutcomeMarket__InsufficientShares();
        uint256 n = i_outcomeSlotCount;
        ...
        for (uint256 i = 0; i < n; i++) {
            uint256 s = s_reserves[i] * sharesToBurn / s_totalShares; // round DOWN (pool's favor)
            sendAmt[i] = s;
            if (s < mergeable) mergeable = s;
        }

        uint256 feeShare = s_sharesOf[msg.sender] * s_accFeePerShare / WAD - s_feeDebt[msg.sender];

        // ---- Effects ----
        for (uint256 i = 0; i < n; i++) {
            s_reserves[i] -= sendAmt[i];
            // One-sided remainder (slice minus the mergeable complete set) → LP's residual position.
            s_balanceOf[i][msg.sender] += sendAmt[i] - mergeable;
        }
        s_collectedFees -= feeShare;
        s_sharesOf[msg.sender] -= sharesToBurn;
        s_totalShares -= sharesToBurn;
        s_feeDebt[msg.sender] = s_sharesOf[msg.sender] * s_accFeePerShare / WAD;
    ...
    function marginalPrice(uint256 outcome) external view returns (uint256) {
        if (s_totalShares == 0) revert MultiOutcomeMarket__PoolNotFunded();
        if (outcome >= i_outcomeSlotCount) revert MultiOutcomeMarket__InvalidOutcome();
        uint256 n = i_outcomeSlotCount;
    ...
    function calcBuyAmount(uint256 outcome, uint256 investmentAmount) public view returns (uint256) {
        ...
        uint256 x = investmentAmount * (FEE_DENOM - i_feeBps) / FEE_DENOM; // net of fee
        uint256 endingBalance = s_reserves[outcome] * WAD;
        for (uint256 j = 0; j < n; j++) {
            if (j != outcome) {
                uint256 r = s_reserves[j];
                endingBalance = _ceilDiv(endingBalance * r, r + x);
            }
        }
        // sharesOut = (reserve_outcome + x) - newReserve_outcome
        return s_reserves[outcome] + x - _ceilDiv(endingBalance, WAD);
    }
    ...
    function calcSellAmount(uint256 outcome, uint256 returnAmount) public view returns (uint256) {
        ...
        return R + _ceilDiv(endingBalance, WAD) - s_reserves[outcome];
    }
    ...
    function buy(uint256 outcome, uint256 investmentAmount, uint256 minSharesOut) external nonReentrant whenOpen {
        ...
        s_collectedFees += fee;
        s_accFeePerShare += fee * WAD / s_totalShares;

        // ---- Effects: every other reserve grows by x; the bought reserve nets +x - sharesOut ----
        for (uint256 i = 0; i < n; i++) {
            if (i == outcome) {
                s_reserves[i] = s_reserves[i] + x - sharesOut;
            } else {
                s_reserves[i] += x;
            }
        }
        s_balanceOf[outcome][msg.sender] += sharesOut;
    ...
    function sell(uint256 outcome, uint256 returnAmount, uint256 maxSharesIn) external nonReentrant whenOpen {
        ...
        if (s_balanceOf[outcome][msg.sender] < sharesIn) revert MultiOutcomeMarket__InsufficientBalance();
        ...
        s_balanceOf[outcome][msg.sender] -= sharesIn;
        for (uint256 i = 0; i < n; i++) {
            if (i == outcome) {
                s_reserves[i] = s_reserves[i] + sharesIn - R;
            } else {
                s_reserves[i] -= R;
            }
        }
    ...
    ///         i_closeTime. This single function settles every poll type:
    ...
    /// @param payouts numerator per outcome; length must equal i_outcomeSlotCount; sum must be > 0.
    function resolve(uint256[] calldata payouts) external {
        if (msg.sender != i_resolver) revert MultiOutcomeMarket__NotResolver();
        if (block.timestamp < i_closeTime) revert MultiOutcomeMarket__NotClosed();
        if (s_payoutDenominator != 0) revert MultiOutcomeMarket__AlreadyResolved();
        if (payouts.length != i_outcomeSlotCount) revert MultiOutcomeMarket__BadPayoutVector();
        ...
        s_payoutNumerators = payouts; // copy calldata vector into storage
        s_payoutDenominator = den;
        emit Resolved(payouts, den);
    }

    /// @notice After resolution, burn ALL your outcome tokens for their fractional collateral value.
    /// @dev payout = Σ_i balance_i * s_payoutNumerators[i] / s_payoutDenominator. For a one-hot result
    ...
    function redeem() external nonReentrant {
        if (s_payoutDenominator == 0) revert MultiOutcomeMarket__NotResolved();
        uint256 n = i_outcomeSlotCount;
        uint256 den = s_payoutDenominator;

        // ---- Effects: tally and zero every outcome balance before paying (CEI) ----
        uint256 totalPayout = 0;
        for (uint256 i = 0; i < n; i++) {
            uint256 bal = s_balanceOf[i][msg.sender];
            if (bal > 0) {
                totalPayout += bal * s_payoutNumerators[i] / den; // round DOWN (pool's favor)
                s_balanceOf[i][msg.sender] = 0;
            }
        }
        if (totalPayout == 0) revert MultiOutcomeMarket__NothingToRedeem();

        // ---- Interaction ----
        i_collateral.safeTransfer(msg.sender, totalPayout);
    ...
    function _maxReserve() internal view returns (uint256 m) {
        uint256 n = i_outcomeSlotCount;
        for (uint256 i = 0; i < n; i++) {
            if (s_reserves[i] > m) m = s_reserves[i];
        }
    }
